...
class Transition:
    ...
    def reduced_electric_matrix_element(self):
        ...
        if self.type == TransitionType.E1:
            ...
            return (
                reduced_electric_dipole_matrix_element(self.A, self.k, J_f, self._ureg)
            ).to("e a0")
        elif self.type == TransitionType.E2:
            J_f = self.state_f.quantum_numbers["J"]
            return reduced_electric_quadrupole_matrix_element(
                self.A, self.k, J_f, self._ureg
            ).to("e a0**2")
        else:
    ...
    def matrix_element(self, mJ_i: int, mJ_f: int):
        if self.type == TransitionType.E1:
            ...
            return dipole_matrix_element(
                self.A, self.k, J_i, J_f, mJ_i, mJ_f, self._ureg
            ).to("a0")
        elif self.type == TransitionType.E2:
            J_i = self.state_i.quantum_numbers["J"]
            J_f = self.state_f.quantum_numbers["J"]
    ...
    def electric_matrix_element(self, mJ_i: int, mJ_f: int):
        if self.type == TransitionType.E1:
            J_i = self.state_i.quantum_numbers["J"]
            J_f = self.state_f.quantum_numbers["J"]
            return electric_dipole_matrix_element(
                self.A, self.k, J_i, J_f, mJ_i, mJ_f, self._ureg
            ).to("e a0")
        elif self.type == TransitionType.E2:
            J_i = self.state_i.quantum_numbers["J"]
            J_f = self.state_f.quantum_numbers["J"]
            return electric_quadrupole_matrix_element(
                self.A, self.k, J_i, J_f, mJ_i, mJ_f, self._ureg
            ).to("e a0**2")
        else:
            raise NotImplementedError(
    ...
    def sublevels_field_sentitivity(self):
        return {
            (mi, mf): (
                field_sensitivity(self.state_f.g, mf, self._ureg)
                - field_sensitivity(self.state_i.g, mi, self._ureg)
            )
            for (mi, mf) in self.sublevels
        }

    def sublevels_zeeman_shift(self, B: pint.Quantity):
        return {
            (mi, mf): (
                zeeman_shift(self.state_f.g, mf, B, self._ureg)
                - zeeman_shift(self.state_i.g, mi, B, self._ureg)
            ).to("MHz")
            for (mi, mf) in self.sublevels
        }
    ...
    def saturation_intensity(self):
        return (
            self._ureg("pi*planck_constant*c/3") * self.A / (self.wavelength**3)
        ).to("mW/cm^2")
    ...
    def type(self) -> TransitionType:
        ...
        if (
            self.state_i.coupling == Coupling.LS
            and self.state_f.coupling == Coupling.LS
        ):
