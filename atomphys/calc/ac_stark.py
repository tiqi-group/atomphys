...
def ac_stark_shift(
    ...
    for transition in state.transitions_from:
        ...
        for i in range(int(state_f.quantum_numbers['J']*2+1)):
            ...
            try:
                Omega = complex((Rabi_Frequency(E_field=El_field, transition=transition, mJ_i=mJ_i, mJ_f=mJ_f, _ureg=_ureg)).to('MHz').magnitude)*_ureg('MHz')
                delta_E += _ureg('hbar')/4*((Omega*np.conj(Omega))/(-transition.angular_frequency - omega_field)+(Omega*np.conj(Omega))/(-transition.angular_frequency + omega_field))
            except:
                raise
    ...
    if wavelengths is None:
        ...
        return np.array([complex((delta_single_E.to('k*mK')).magnitude) for delta_single_E in delta_E])*u('k*mK')
...
def polarizability(
    ...
    AC_stark = ac_stark_shift(state=state, mJ=mJ, El_field=El_field, _ureg=_ureg, wavelengths=wavelengths)
