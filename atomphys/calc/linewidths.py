...
from atomphys.utils.utils import halfint
...
def transition_specific_linewidth(transition: Transition, mJ_i: float, mJ_f: float, _ureg: pint.UnitRegistry):
    ...
    if transition.type == TransitionType.E1:
        ...
        lo = 0 * _ureg('MHz')
        for q in [-1, 0, 1]:
            lo += pre * rd_sq * w3j(halfint(J_f), 1, halfint(J_i), halfint(-mJ_f), q, halfint(mJ_i))**2
        ...
        lo = 0 * _ureg('MHz')
        for q in [-2, -1, 0, 1, 2]:
            lo += pre * rd_sq * w3j(halfint(J_f), 2, halfint(J_i), halfint(-mJ_f), q, halfint(mJ_i))**2
    else:
