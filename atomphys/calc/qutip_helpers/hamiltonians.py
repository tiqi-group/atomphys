...
def hamiltonian_bare(states: list[State], _ureg: pint.UnitRegistry):
    ...
    ket = [
        qutip.basis(total_number_of_states, i) for i in range(total_number_of_states)
    ]
            ...
            for i in range(int(2 * J_i + 1)):
                ...
                for f in range(int(2 * J_f + 1)):
                    ...
                    H += (
                        1
                        / 2
                        * complex(abs(Omega_ij).magnitude)
                        * (ket[index_i] * ket[index_f].dag())
                    )
                    H += (
                        1
                        / 2
                        * np.conj(complex(abs(Omega_ij).magnitude))
                        * (ket[index_f] * ket[index_i].dag())
                    )
...
def hamiltonian_interaction_without_diagonals(
    states: list[State],
    fields: dict[ElectricField, list[Transition]],
    _ureg: pint.UnitRegistry,
):
